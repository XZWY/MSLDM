...
class ExponentialDistribution(Distribution):
    def __init__(self, rate: float, sigma_max: float = 5.0):
        self.rate = rate
        self.sigma_max = sigma_max

    def __call__(self, num_samples: int, device: torch.device = torch.device("cpu")):
        samples = torch.distributions.Exponential(self.rate).sample((num_samples,))
        
        # Resample for values exceeding sigma_max
        while True:
            resample_mask = samples > self.sigma_max
            if not resample_mask.any():
                break
            resamples = torch.distributions.Exponential(self.rate).sample((resample_mask.sum(),))
            samples[resample_mask] = resamples
...
class Diffusion(nn.Module):
    ...
    def forward(self, x: Tensor, noise: Tensor = None, **kwargs) -> Tensor:
        ...
        # losses[:, 160:240, :] = losses[:, 160:240, :] * 2.5
        # losses[:, 240:320, :] = losses[:, 240:320, :] * 1.5
